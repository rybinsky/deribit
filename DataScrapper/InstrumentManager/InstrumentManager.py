...
from ..Utils.AvailableConfigurationRoot import ConfigRoot
from ..Utils.MSG_LIST import get_positions_request, auth_message
...
class InstrumentManager(Thread):
    ...
    async def process_validation(self, callback: dict):
        ...
        if all(key in callback for key in self._position_keys):
            ...
            if self.managed_instruments[instrument_name] != callback["size"]:
                logging.error(f"Instrument {instrument_name} has mismatch in sizes | "
                              f"Recorded = {self.managed_instruments[instrument_name].user_position} | "
                              f"Real (Deribit Info) = {callback['size']}")
    ...
    async def update_order_book(self, callback):
        ...
        Фактически заносит информацию об изменении ордербука в tmp хранилища AbstractInstrument,
        и передает абстрактный инструмент стратегии
        :param callback:
        :return:
        """
        ...
        logging.info(f"Update orderBook at Instrument: "
                     f"{self.managed_instruments[_order_book_change['instrument_name']]}")
    ...
    async def update_trade(self, callback):
        ...
        Фактически заносит информацию о сделке в tmp хранилища AbstractInstrument, и
        передает абстрактный инструмент стратегии
        :param callback:
        :return:
        """
    ...
    async def change_user_instrument_position(self, position_change: float, instrument_name: str, increment=True):
        ...
        logging.info(f"Update user position ({self.managed_instruments[instrument_name].user_position}) at instrument: "
                     f"({self.managed_instruments[instrument_name].instrument_name})")
